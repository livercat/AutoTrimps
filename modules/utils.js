...
function getPageSetting(setting, defaultValue) {
    ...
    let value;
    if (type === 'boolean') {
        value = autoTrimpSettings[setting].enabled;
    } else if (type === 'multiValue') {
        value = Array.from(autoTrimpSettings[setting].value).map(x => parseInt(x));
    } else if (['value', 'valueNegative'].includes(type)) {
        value = parseFloat(autoTrimpSettings[setting].value);
    } else if (type === 'multitoggle') {
        value = parseInt(autoTrimpSettings[setting].value);
    } else if (type === 'dropdown') {
        value = autoTrimpSettings[setting].selected;
    }
...
function setPageSetting(setting, value) {
    ...
    } else if (['value', 'valueNegative', 'textValue', 'multiValue', 'multitoggle'].includes(type)) {
        autoTrimpSettings[setting].value = value;
    } else if (type === 'dropdown') {
        autoTrimpSettings[setting].selected = value;
    }

    if (['boolean', 'multitoggle'].includes(type)) {
        document.getElementById(setting).setAttribute('class', 'noselect settingsBtn settingBtn' + value);
    }
    return true;
}
